...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    assert.strictEqual(typeof objetoRetornado, 'object');//{ fetchMenu: () => {}, food: {}, drink: {}, consumption: [], order: () => {}, pay: () => {} });
    ...
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    const somaDosPreçosDosPedidos = 12.87;//(3.9 + 3.9 + 3.9) = 11.7 + 10%
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    assert.strictEqual(objetoRetornado.pay(objetoRetornado.consumption), somaDosPreçosDosPedidos);
    // ```
